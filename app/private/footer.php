<footer class="footer-wrapper">
  <div class="footer-content">
    <div class="footer-content-logo">
      <a href="<?php echo BASE_URL . "/" ?>">
        <img src="<?php echo BASE_URL . "/public/img/logo.svg" ?>" alt="Logo - image of cat" width="75"/>
      </a>
    </div>
    <div class="footer-content-links">
      <ul class="nav-links">
        <li>
          <a href="<?php echo BASE_URL . "/" ?>">Home</a>
        </li>
        <li>
          <a href="<?php echo BASE_URL . "/donate" ?>">Donate</a>
        </li>
        <li>
          <a href="<?php echo BASE_URL . "/pets" ?>">Pets</a>
        </li>
        <li>
          <a href="<?php echo BASE_URL . "/news" ?>">News</a>
        </li>
        <li>
          <a href="<?php echo BASE_URL . "/about-us" ?>">About us</a>
        </li>
        <li>
          <a href="<?php echo BASE_URL . "/contact-us" ?>">Contact us</a>
        </li>
        <li>
          <a href="<?php echo BASE_URL . "/supporters/signin" ?>">Sign in</a>
        </li>
      </ul>
      <ul class="nav-links">
        <li>
          <a href="https://www.youtube.com/channel/UC7DW3cdAYBTn5I6Swh_RZYw" class="social-media-link">
            <img src="<?php echo BASE_URL . "/public/img/youtube.svg" ?>"
                 alt="YouTube logo - Link to __'s YouTube channel"
                 width="22">
            YouTube
          </a>
        </li>
        <li>
          <a href="https://www.facebook.com/UniversityOfSouthWales" class="social-media-link">
            <img src="<?php echo BASE_URL . "/public/img/facebook.svg" ?>"
                 alt="Facebook logo - Link to __'s Facebook account" width="22">
            Facebook
          </a>
        </li>
        <li>
          <a href="https://x.com/UniSouthWales" class="social-media-link">
            <img src="<?php echo BASE_URL . "/public/img/x.svg" ?>" alt=" X logo - Link to __'s X account" width="22">
            X
          </a>
        </li>
        <li>
          <a href="https://www.instagram.com/unisouthwales/" class="social-media-link">
            <img src="<?php echo BASE_URL . "/public/img/instagram.svg" ?>" alt=" Instagram logo - Link to __'s Instagram
            account" width="22">
            Instagram
          </a>
        </li>
      </ul>
    </div>
  </div>
</footer>
